...
void loop() { 
 ...
 for (int pin_number = STARTPIN; pin_number <= ENDPIN; pin_number++) { 
    ...
    if (button_states[pin_number] == 1 && time_passed >= BUTTON_IGNORE_TIME) {      
...
void loop() {   for (int pin_number = STARTPIN; pin_number <= ENDPIN; pin_number++) {    
